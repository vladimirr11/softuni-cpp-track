...
public:
    ...
    Organism(double weight) : weight(weight) {
...
public:
    Animal(double movementSpeed, double weight) 
        : // NOTE: Organism ctor will ONLY be called when creating an Animal -
          // it won't be called when creating any inheriting class (like GMO)
        Organism(weight), movementSpeed(movementSpeed) {
...
public:
    Plant(bool hasSeeds, double weight) 
        : // NOTE: Organism ctor will ONLY be called when creating a Plant -
          // it won't be called when creating any inheriting class (like GMO)
        Organism(weight), hasSeeds(hasSeeds) {
...
public:
    GMO(double movementSpeed, bool hasSeeds, double weight) 
        : // NOTE: We need to call Organism, because due to virtual inheritance
          // Animal and Plant WON'T call it (that's why we're passing 0 weight to them -
          // the only value that will matter is the one passed to the Organism)
        Organism(weight), Animal(movementSpeed, 0), Plant(hasSeeds, 0) {}
