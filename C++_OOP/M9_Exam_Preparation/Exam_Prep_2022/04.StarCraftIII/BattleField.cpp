...
void BattleField::generateTerranFleet(const std::string& fleetComposition) {
    ...
    for (int i = 0; i < FLEET_SIZE; ++i) {
        ...
        case 'b':
            _terranFleet.emplace_back(new BattleCruser(AirShipType::BATTLE_CRUSER,
                                        BATTLE_CRUSER_HEALTH, BATTLE_BRUSER_DAMAGE, i));
            break;

        case 'v':
            _terranFleet.emplace_back(new Viking(AirShipType::VIKING, VIKING_HEALTH, VIKING_DAMAGE, i));
            break;
        }
    }
}
