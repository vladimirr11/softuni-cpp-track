...
CommandExecutor::CommandExecutor(const int memoryPoolSize) : _memoryPool(memoryPoolSize) {
...
void CommandExecutor::executeRequestMemory(const int nodeSize) {
    ...
    // REQUEST_FAILURE_NOT_ENOUGH_MEMORY == ERROR_CODE 
    else {
...
void CommandExecutor::executeIncrNodeDataValue(const int incrValue, const int nodeIdx) {
    for (int i = 0; i < _usedNodes[nodeIdx].size; ++i) {
