...
size_t getSingleCommandSize(const CommandType commandType) {
    ...
    case CommandType::LONG_LONG:
        ...
        break;
    default:
        std::cerr << "Warning, received CommandType::UNKNOWN. Returning 0." << std::endl;
        break;
    }
...
ErrorCode parseData(const std::string& commands, const char* rawDataBytes,
                    const size_t rawDataBytesCount, std::vector<long long>& outParsedNumbers) {
    size_t parsedSize = 0;
    char* rawDataPtr = const_cast<char*>(rawDataBytes);

    if ((commands.empty()) || (rawDataBytesCount == 0) || (rawDataBytes == nullptr)) {
    ...
    for (size_t i = 0; i < commands.size(); ++i) {
        ...
        // parse the data
        outParsedNumbers.push_back(parseSingleNumber(commandType, rawDataPtr));
