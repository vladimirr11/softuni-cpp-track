...
CommandAction getCommand(const std::string& command, const size_t memorySize,
                         size_t& outIndex) {
    if (command == "Idle") {
...
ErrorCode executeCommand(const std::string& command, std::vector<int*>& memory) {
    ...
    size_t index = 0;
    const CommandAction commandAction = getCommand(command, memory.size(), index);
