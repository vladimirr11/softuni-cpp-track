...
static bool isValidMove(const std::vector<Point>& obstacles, 
    ...
    const auto selfIt = std::find_if(obstacles.begin(), obstacles.end(), 
                                     [&pos](const Point& obstacle) {
                                     return obstacle == pos;});
    
    if (selfIt != obstacles.end()) {
        return false;
    }
...
static bool shouldPowerUp(const std::vector<Point>& powerUps, const Point& pos, size_t collisionIndx) {
    ...
    const auto it = std::find_if(powerUps.begin(), powerUps.end(), 
                                 [&pos](const Point& node) {
                                 return node == pos;});
