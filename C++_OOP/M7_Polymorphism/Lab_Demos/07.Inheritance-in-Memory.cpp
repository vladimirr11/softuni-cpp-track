...
public:
    ...
    Spider(int numLegs, float weight)
        : Organism(weight, false, true), numLegs(numLegs) {}
};
...
int main() {
    ...
    // NOTE: cow has an actual size of 6 bytes, but padding/alignment will cause it to grow to next power of 2,
    ...
    std::cout << "allignof Organism = " << alignof(Organism) << std::endl;
    std::cout << "sizeof cow = " << sizeof(cow) << std::endl;
    std::cout << "allignof Spider = " << alignof(Spider) << std::endl;
    std::cout << "sizeof spider = " << sizeof(s) << std::endl;
