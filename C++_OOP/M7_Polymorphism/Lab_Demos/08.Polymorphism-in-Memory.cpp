...
namespace Organisms {
    ...
    public:
        Organism(double weight, bool eatsPlants, bool eatsAnimals) 
            : eatsPlants(eatsPlants), eatsAnimals(eatsAnimals), weight(weight) {}
    ...
    public:
        ...
        Sheep(bool isBlack, double weight) : 
            Organism(weight, true, false), isBlack(isBlack) {}
    ...
    public:
        ...
        string getInfo() const {
            ostringstream stream;
            stream << this->numLegs << "-legged"
                   << " Spider, " << Organism::getInfo();
            return stream.str();
        }
    };
}
