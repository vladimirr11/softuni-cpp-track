...
Field::Field(const FieldConfig& cfg) 
    : _ROWS(cfg.rowsCount), _COLS(cfg.colsCount), _obstacles(cfg.obstacles), _powerUps(cfg.powerUps),
    _fieldNodes(_ROWS), _snake(cfg.rowsCount, cfg.colsCount, cfg.snakeStartPos) {
    updateFieldState();
    printField(true);
}
