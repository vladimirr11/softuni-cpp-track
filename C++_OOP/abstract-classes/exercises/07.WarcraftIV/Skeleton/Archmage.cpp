...
Archmage::Archmage(const std::string& name, const int maxMana, 
                   const int baseManaRegenRate, const int manaRegenModifier) :
...
void Archmage::castSpell(const SpellType spell) {
    if (_currMana >= _spells[spell].manaCost) {
        ...
        std::cout << _name << " casted " << _spells[spell].name << 
        " for " << _spells[spell].manaCost << " mana" << std::endl;
