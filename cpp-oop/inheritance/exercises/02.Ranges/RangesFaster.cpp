...
bool inRange(int currInt, std::map<int, std::shared_ptr<Ranges>>& fromRangesMap,
    ...
    auto fromIt = fromRangesMap.lower_bound(currInt); /// find first range start higher than value
    ...
    auto toIt = toRangesMap.upper_bound(currInt);
    if (toIt == toRangesMap.end()) {
